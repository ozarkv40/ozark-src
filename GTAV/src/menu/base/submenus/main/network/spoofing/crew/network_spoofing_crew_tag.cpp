...
using namespace network::spoofing::crew::tag::vars;

namespace network::spoofing::crew::tag::vars {
	variables m_vars;

	void spoof(bool reset, const char* name) {
		network_spoofing_crew_tag_menu* _this = network_spoofing_crew_tag_menu::get();
		...
		if (reset) {
			...
			if (m_vars.m_save_to_config) {
				util::config::write_string(_this->get_submenu_name_stack(), "Tag", "");
			}
		...
		if (m_vars.m_save_to_config) {
			util::config::write_string(_this->get_submenu_name_stack(), "Tag", name);
		}

		menu::notify::stacked(_this->get_name().get(), _this->get_string("Spoofing crew tag to") + " " + name);
	}
}

void network_spoofing_crew_tag_menu::load() {
	set_name("Crew Tag");
	...
	add_option(submenu_option("Presets")
		...
		.add_click([] {
			network_spoofing_crew_presets_menu::get()->set_parent<network_spoofing_crew_tag_menu>();
			network::spoofing::crew::presets::vars::m_vars.m_editing = &m_vars.m_temp_name;
		}));

	add_option(button_option("Input Crew Tag")
		.add_translate()
		.add_keyboard("Enter crew tag", 24, [](button_option* _this, const char* result) { m_vars.m_temp_name = std::string(result); }));
	...
	if (m_vars.m_save_to_config) {
		m_vars.m_spoofed_name = util::config::read_string(get_submenu_name_stack(), "Tag", m_vars.m_spoofed_name);
		m_vars.m_temp_name = m_vars.m_spoofed_name;
		spoof(false, m_vars.m_temp_name.c_str());
	}

	add_string("Spoofing crew tag to");
}

void network_spoofing_crew_tag_menu::update() {
	snprintf(m_vars.m_preview_name, 50, "Apply: %s", m_vars.m_temp_name.c_str());
	m_vars.m_preview_button->set_name(m_vars.m_preview_name);
}

void network_spoofing_crew_tag_menu::update_once() {}

void network_spoofing_crew_tag_menu::feature_update() {}

network_spoofing_crew_tag_menu* g_instance;
network_spoofing_crew_tag_menu* network_spoofing_crew_tag_menu::get() {
	if (g_instance == nullptr) {
		g_instance = new network_spoofing_crew_tag_menu();
		g_instance->load();
